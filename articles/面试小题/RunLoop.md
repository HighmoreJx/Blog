...
RunLoop见名知意,即运行一个循环.  
哈?跑一个循环什么时候也成了一个了不得的技术了?  

sorry啊.循环就是那么了不起......  

我们刚接触编程的时候, 第一个程序一般都是"hello world". 回想下,是不是黑窗口打印了"hello world",然后一切就结束了.要是我们的GUI程序像"hello world"一样, 那不搞笑了?点一个按钮,APP直接玩完.

这个时候,我们的主角循环就登场了, 但是单纯的循环一直跑的话,那将极其耗费资源.所以就需要这个循环更加高端.不需要做啥事情的时候就老老实实睡眠,该干活的时候就麻利的醒来做事.这个无良压榨的方式美其名曰 - 事件处理模型.

这个事件模型不是iOS平台下独有的, 各个平台都有自己的一套.iOS平台下对这套模式的实现就叫 - RunLoop.翻译成中文就是跑循环.哈哈,这大概就是传说中的语文是体育老师教的.
...
了解了其本质是个while循环以后,RunLoop与线程的关系就很清晰了.
