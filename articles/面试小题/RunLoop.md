...
: 每次runloop进入时的activity,runloop每一次进入一个mode,就通知一次外部kCFRunLoopEntry,之后会一直以该mode进行,直到当前mode被终止,进而切换到其他mode,并在此通知kCFRunLoopEntry.
...
上面的流程图是mrpeak博客上取得,感觉很清晰易懂.下面会结合代码(官网可能会更新)来体会一下这个流程.源代码太多,这边会截取关键的部分.   
